<%@ page import="java.security.MessageDigest" %>
<%@ page import="java.security.NoSuchAlgorithmException" %>
<%--<%@ page import="com.liuyanzhao.ssm.blog.util.MyUtils" %>--%>
<%@ page language="java" contentType="text/html;charset=UTF-8"
         pageEncoding="UTF-8" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
<%@ taglib prefix="rapid" uri="http://www.rapid-framework.org.cn/rapid" %>

<rapid:override name="title">
    - 回复评论
</rapid:override>
<rapid:override name="header-style">
    <style>
        /*覆盖 layui*/

        .layui-btn {
            margin: 2px 0 !important;
        }
    </style>
</rapid:override>

<rapid:override name="content">

    <blockquote class="layui-elem-quote">
        <span class="layui-breadcrumb" lay-separator="/">
              <a href="/admin">后台</a>
              <a href="/admin/comment">评论列表</a>
              <a><cite>回复评论</cite></a>
        </span>
    </blockquote>

    <form class="layui-form" method="post" id="myForm" action="/admin/comment/replySubmit">
            <%--被评论的评论的id--%>
        <input type="hidden" name="commentPid" value="${comment.commentId}">
            <%--被评论的用户id--%>
        <input type="hidden" name="commentPuserId" value="${comment.commentUserId}">
            <%--被评论文章的id--%>
        <input type="hidden" name="commentArticleId" value="${comment.commentArticleId}">
            <%--被评论用户的昵称--%>
        <input type="hidden" name="commentPname" value="${comment.user.userNickname}">
        <div class="layui-form-item layui-form-text">
            <label class="layui-form-label">回复用户</label>
            <div class="layui-input-block">
                    <%--被评论的用户的昵称--%>
                <textarea class="layui-textarea" disabled>${comment.user.userNickname}</textarea>
            </div>
        </div>


        <div class="layui-form-item layui-form-text">
            <label class="layui-form-label">原内容</label>
            <div class="layui-input-block">
                <textarea class="layui-textarea" disabled>${comment.commentContent}</textarea>
            </div>
        </div>

        <div class="layui-form-item layui-form-text">
            <label class="layui-form-label">我的回复</label>
            <div class="layui-input-block">
                    <%--回复的内容--%>
                <textarea name="commentContent" class="layui-textarea"></textarea>
            </div>
        </div>

        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn" lay-submit="" lay-filter="demo1">回复</button>
                <button type="reset" class="layui-btn layui-btn-primary">重置</button>
            </div>
        </div>

    </form>


</rapid:override>
<rapid:override name="footer-script">
    <script>

    </script>
</rapid:override>

<%@ include file="../Public/framework.jsp" %>
