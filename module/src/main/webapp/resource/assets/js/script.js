...
$(".comment-reply-link").click(function () {
    ...
    //添加被评论的id
    var commentPuserId = $(this).parents('.comment-author').find("b.commentPuserId").text();
    window.alert(commentPuserId);
    $("input[name=commentPuserId]").attr("value", commentPuserId);
    ...
    if ($.cookie("viewId") != articleId || $.cookie("viewId") == null) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $.cookie(
                    //设置view id等于articleId，使得避免每次打开文章都增加一次阅读量，避免刷阅读量。
                    "viewId",
